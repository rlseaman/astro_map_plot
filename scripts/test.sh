...
"$MAPPLOT" -h | grep -A 30 "Available projections:" | head -20

# Test 3: Create a simple map
echo ""
echo "Test 3: Creating test map..."
"$MAPPLOT" -p mollweide -o "$OUTPUT_DIR/test_map.png" "$DATA_DIR/example_cities.txt"
...
"$MAPPLOT" -p hammer -g -o "$OUTPUT_DIR/test_grid.png" "$DATA_DIR/example_cities.txt"
...
"$MAPPLOT" -p robinson -s 10 -o "$OUTPUT_DIR/test_sizes.png" "$DATA_DIR/example_cities_pop.txt"
...
"$MAPPLOT" -p eckert4 --cmap coolwarm --cbar -o "$OUTPUT_DIR/test_colormap.png" "$DATA_DIR/example_temperature.txt"
